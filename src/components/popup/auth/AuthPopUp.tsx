...
const AuthPopUp = ({
  ...
  setIsAuthNotificationVisible: (visible: {
    status: boolean;
    message: string;
  }) => void;
}) => {
    ...
    <View className="w-full h-80 bg-zinc-900 rounded-[35px] p-6 border border-zinc-700 flex justify-between">
      <View className="w-full flex-row items-center justify-between">
        <Text className="text-zinc-300 text-2xl font-bold">
          Alert <Text className="text-red-400"> (Error)</Text>{' '}
        </Text>
      </View>
      ...
      <TouchableOpacity
        className="w-full bg-zinc-600 rounded-full py-5 flex items-center justify-center"
        onPress={() => {
